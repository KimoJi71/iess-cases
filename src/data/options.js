...
const PROJECT_STAGES = ['現勘', '立案時間', '工程發包作業', '設備訂貨作業', '廠商驗收作業', '客戶驗收', '發票請款作業'];
...
const EQUIP_STATUS_OPTIONS = ['運轉中', '達年限', '已汰換'];
const EQUIP_STATUS_RETIRED = '已汰換';
const EQUIP_STATUS_OVER_AGE = '達年限';
// 設備等級：影響 A/B 服務等級叫修單是否計入績效積分。於「設備管理」逐台設定。
const EQUIPMENT_LEVEL_OPTIONS = ['一般設備', '增額設備'];
// 設備未存等級時的預設值（避免依賴陣列順序）
const DEFAULT_EQUIPMENT_LEVEL = '一般設備';
...
// 風管工程 現勘表選項
const DUCT_HOSE_MATERIALS = ['保溫軟管(玻璃棉)', '無保溫軟管(鋁箔)', '螺旋風管(鍍鋅鐵)', '防火保溫軟管(玻璃棉)']; // 風管／米 材質（單選）
const DUCT_HOSE_PIPES = ['4"', '6"', '8"', '10"', '12"']; // 風管／米 管徑
...
// 案件排程專用選項
const SCHEDULE_ASSIGNEE_OPTIONS = ['A組', 'B組', 'C組', 'D組'];
const SCHEDULE_WORK_CATEGORY_OPTIONS = ['一般叫修', '緊急叫修', '保養清潔', '其他', '保養'];

// 報表統計專用選項
const PERFORMANCE_ASSIGNEES = ['A組', 'B組', 'C組', 'D組'];
...
// 系統權限專用選項
const ACCOUNT_ASSIGNEE_OPTIONS = ['A組', 'B組', 'C組', 'D組'];
...
const PERMISSION_TREE = [
  ...
  },
  {
    ...
    children: [
      ...
      {
        id: '基礎資料設定',
        children: ['服務等級管理', '處理方式與積分管理', '設備分類管理', '績效區域管理']
      },
      {
        id: '保養作業',
        children: ['保養分配']
