    ...
    function getFilteredAccounts() {
      ...
      if (kw) {
        list = accounts.filter(function (a) {
          return [a.name, a.username, a.email, AccountUtils.formatDistricts(a.districts), a.assignee]
            .filter(Boolean)
    ...
    return stateful(function (rerender) {
      ...
      function handleDelete(id) {
        ...
        if (target && target.assignee) {
          showToast('帳號已刪除，並已從「' + target.assignee + '」成員名單中移除');
        } else {
          showToast('帳號已刪除');
        }
      }
          ...
          h('table', { className: 'w-full text-left text-sm text-gray-600 whitespace-nowrap select-none' },
            h('thead', { className: 'bg-gray-50 text-gray-700 border-b' },
              h('tr', null,
                ...
                h('th', { className: 'p-3 font-semibold' }, '負責行政區域'),
