...
(function () {
  ...
  function EditProjectForm(props) {
    ...
    function ReadOnlyField(p) {
      return h('div', {
        className: p.fullWidth ? 'col-span-full' : ''
      }, h('span', {
    ...
    return stateful(function (rerender) {
      ...
      return h('div', {
        ...
        label: '服務等級',
        value: formData.details && formData.details.serviceLevel
      }), h('div', {
        className: 'col-span-2 md:col-span-4'
      }, ReadOnlyField({
        label: '門市地址',
        value: formData.details && formData.details.storeAddress
      })), ReadOnlyField({
        label: '工程聯絡人',
        value: formData.details && formData.details.contactPerson
      }), ReadOnlyField({
        label: '建議施作單位',
        value: formData.details && formData.details.suggestedContractor
      }), ReadOnlyField({
        label: '進場日期',
        value: formData.details && formData.details.entryDate
      }), ReadOnlyField({
        label: '立案日期',
        value: formData.creationDate
      }), ReadOnlyField({
        label: '其他事項說明',
        value: formData.details && formData.details.remarks,
        fullWidth: true
      }))), h('section', {
        className: 'bg-white p-6 rounded-lg shadow-sm border border-gray-100'
      }, h('h3', {
        ...
        className: 'divide-y divide-gray-100'
      }, !formData.details || !formData.details.equipment || formData.details.equipment.length === 0 ? h('tr', null, h('td', {
        colspan: '7',
        className: 'text-center p-6 text-gray-400'
      }, '無設備資料')) : formData.details.equipment.map(function (eq) {
        return h('tr', {
          key: eq.id,
          className: 'hover:bg-gray-50'
        }, h('td', {
