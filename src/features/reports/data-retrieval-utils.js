...
(function () {
  ...
  function mapRepairRow(c, stores) {
    return {
      ...
      '客戶名稱': c.customerName || '—',
      '門市名稱': c.storeName || '—',
      '行政區域': resolveRepairArea(c, stores) || '—',
      '客戶分級': c.serviceLevel || '—',
      '工項分類': c.workCategory || '—',
      '叫修項目': c.repairItem || '—',
      '叫修原因': c.repairReason || '—',
  ...
  function getColumns(caseType) {
    ...
    if (caseType === '維修') {
      return ['叫修時間', '案件編號', '客戶名稱', '門市名稱', '行政區域', '客戶分級', '工項分類', '叫修項目', '叫修原因', '故障描述', '實際原因', '維修人員', '處理狀態', '後續處理狀態', '後續處理時間', '結案狀態', '結案日期'];
    }
    return ['基準日期', '客戶名稱', '門市名稱', '縣市', '行政區', '客戶分級', '保養狀態', '維修人員', '預約日期', '完成日期', '結案狀態'];
  }
