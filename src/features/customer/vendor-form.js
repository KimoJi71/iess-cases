/*
 * features/customer/vendor-form.js — 客戶建檔：新增/編輯廠商表單
 * props: { vendors, setVendors, setView, showToast, targetCase }
 */
(function () {
  'use strict';
  ...
  function VendorForm(props) {
    ...
    var formData = {
      ...
      type: (targetCase && targetCase.type) || VENDOR_TYPE_OPTIONS[0] || '',
    ...
    return stateful(function (rerender) {
      ...
      function handleSubmit(e) {
        ...
        var savedContacts = contacts.map(function (ct) { return Object.assign({}, ct); });
        if (isEdit) {
          setVendors(vendors.map(function (v) {
            return v.id === targetCase.id
              ? Object.assign({}, v, formData, { contacts: savedContacts })
              : v;
          }));
          showToast('廠商資料更新成功');
        } else {
          var newVendor = Object.assign({ id: 'VEND' + Date.now() }, formData, {
            contacts: savedContacts,
            createdDate: todayDate
          });
          setVendors([newVendor].concat(vendors));
          showToast('廠商新增成功');
        }
        setView('vendor-list');
      }
        ...
        h('form', { onSubmit: handleSubmit, className: 'p-4 sm:p-6 space-y-6 sm:space-y-8 bg-gray-50 rounded-b-lg' },
          h('section', { className: 'bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-100' },
            ...
            h('div', { className: 'grid grid-cols-1 md:grid-cols-3 gap-6' },
              ...
              h('div', null,
                h('label', { className: 'block text-sm mb-1' }, '廠商種類'),
                h('select', {
                  name: 'type',
                  value: formData.type,
                  onChange: handleChange,
