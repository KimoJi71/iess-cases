/*
 * core/icons.js — 內嵌 SVG 圖示（取代 lucide-react）
 *
 * 每個圖示為一個函式：Icons.Search({ className: 'h-5 w-5' }) 回傳 <svg>。
 * 樣式沿用 lucide 規格：viewBox 0 0 24 24、stroke currentColor、寬度 2、圓角端點。
 */
(function (global) {
  'use strict';
  ...
  var PATHS = {
    ...
    RefreshCw: '<path d="M3 12a9 9 0 0 1 15.5-6.2L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-15.5 6.2L3 16"/><path d="M3 21v-5h5"/>',
  ...
  function makeIcon(name, inner) {
    return function (props) {
      ...
      svg.setAttribute('stroke-width', props.strokeWidth || '2');
      svg.setAttribute('stroke-linecap', 'round');
      svg.setAttribute('stroke-linejoin', 'round');
      svg.setAttribute('aria-hidden', 'true');
      if (props.className) svg.setAttribute('class', props.className);
      if (props.fill) svg.setAttribute('fill', props.fill);
      svg.innerHTML = inner;
  ...
  Object.keys(PATHS).forEach(function (name) {
    Icons[name] = makeIcon(name, PATHS[name]);
  });

  global.IESS = global.IESS || {};
  global.IESS.Icons = Icons;
})(window);
