...
| 年中主檔變動 | 預設凍結，提供「重新同步保養區間」按鈕重拍骨架；已填格子一律保留 |
...
2. **指派人員** select — 行為不變。
3. 右側：客戶數統計，以及有快照時顯示的「重新同步保養區間」按鈕。
...
**僅當選定年度為當年時**（`selectedYear === new Date().getFullYear()`）才跑 `diffSnapshot`、才顯示提示條與「重新同步保養區間」按鈕。過去年度改顯示中性灰字：
...
「重新同步保養區間」按鈕開確認 Modal，內容為 `formatDiffSummary(diff)` 與孤兒格子預估數（同步後會有幾格落在區間外），確認後：
...
6. 接續 4：對 2026 按「重新同步保養區間」→ 區段變兩段，原本第 3、4 次的格子仍在、標為孤兒、點擊只能刪除。
